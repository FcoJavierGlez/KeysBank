 ...
 */
{
    const init = () => {
        ...
        NICK_INPUT.addEventListener( "keyup", () => NICK_INPUT.value = cleanInput(NICK_INPUT.value) );
        NICK_INPUT.addEventListener( "copy", e => e.preventDefault() );
        NICK_INPUT.addEventListener( "paste", e => e.preventDefault() );

        PASS_INPUT.addEventListener( "keyup", () => PASS_INPUT.value = cleanInput(PASS_INPUT.value) );
        ...
        if (location.href.match(/\?register$/)?.input !== undefined) {
            ...
            REP_PASS_INPUT.addEventListener( "keyup", () => REP_PASS_INPUT.value = cleanInput(REP_PASS_INPUT.value) );
            REP_PASS_INPUT.addEventListener( "copy", e => e.preventDefault() );
            REP_PASS_INPUT.addEventListener( "paste", e => e.preventDefault() );

            EMAIL_INPUT.addEventListener( "keyup", () => EMAIL_INPUT.value = cleanInput(EMAIL_INPUT.value) );
        }
